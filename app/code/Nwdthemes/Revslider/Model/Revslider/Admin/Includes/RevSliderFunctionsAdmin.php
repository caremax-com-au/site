		...
		if(move_uploaded_file($path, $new_path)){
			$upload_id = wp_insert_attachment(
				array(
					...
					'post_name'		 => $this->_frameworkHelper->sanitize_title_with_dashes(str_replace('_', '-', $file_name)),
					'post_content'	 => '',
					'post_status'	 => 'inherit'
				),
				$new_path
			...
			@$this->_frameworkHelper->wp_update_attachment_metadata($upload_id, $this->_frameworkHelper->wp_generate_attachment_metadata($upload_id, $new_path));

			//$meta = $this->_frameworkHelper->wp_get_attachment_metadata( $attachment->ID );

			$img_dim = @$this->_frameworkHelper->wp_get_attachment_image_src($upload_id, 'full');
			...
			foreach($images as $key => $image){
				...
				if($objlib->_is_object($image)){
					...
					if(!empty($img_data)){
						...
						if ($this->_frameworkHelper->getImagesHelper()->image_to_url($image) === $return[$key]['orig']['url']) $return[$key]['orig']['default'] = true;
